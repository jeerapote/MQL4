...
int start()
  {
         ...
         HAOpen3  = iCustom(Symbol(), PERIOD_H4, "Heiken_Ashi_Smoothed", 2, 4, 2, 1, 2,  1);
         HAClose3 = iCustom(Symbol(), PERIOD_H4, "Heiken_Ashi_Smoothed", 2, 4, 2, 1, 3,  1);
...
int CheckBuySellGrid(){
       ...
       for(int cpt=1;cpt<=LEVELS;cpt++)
       {
         ...
         if(!foundsell && NormalizeDouble(LowerLimit+cpt*INCREMENT*Point,Digits) < Bid - (INCREMENT+StopLevel)*Point){
         
              ticket=OrderSend(Symbol(),OP_SELLSTOP,Lots,NormalizeDouble(LowerLimit+cpt*INCREMENT*Point,Digits),2,0,NormalizeDouble(LowerLimit+cpt*INCREMENT*Point-INCREMENT*Point,Digits),DoubleToStr(Bid,MarketInfo(Symbol(),MODE_DIGITS)),MagicNumber+3,0);
              if(ticket>0){
               lastTradeTime = Time[THIS_BAR];
