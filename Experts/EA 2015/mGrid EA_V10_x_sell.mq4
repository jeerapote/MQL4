   ...
   if(EnableRetracement){
      ...
      for(int i=total-1;i>=0;i--){
       ...
       if(OrderSelect(i, SELECT_BY_POS)==true){
         ...
         if((!retracementDisabled && OrderType() == OP_SELL && Bid > OrderOpenPrice()+(RETRACEMENT+INCREMENT)*Point && lastTradeTime != Time[THIS_BAR])){
       ...
       for(int cpt=1;cpt<=LEVELS;cpt++)
       {
         ...
         for(int i=total-1;i>=0;i--){
          ...
          if( OrderSelect(i, SELECT_BY_POS, MODE_TRADES) ){
            ...
            && OrderOpenPrice() == NormalizeDouble(UpperLimit-cpt*INCREMENT*Point,Digits) 
            || OrderTakeProfit() == NormalizeDouble(UpperLimit-cpt*INCREMENT*Point+INCREMENT*Point,Digits)){
         ...
         if(!foundbuy && NormalizeDouble(UpperLimit-cpt*INCREMENT*Point,Digits)< Bid-(INCREMENT+StopLevel)*Point){
              ...
              ticket=OrderSend(
                                 ...
                                 ,OP_SELLSTOP
                                 ,LOTS
                                 ,NormalizeDouble(UpperLimit-cpt*INCREMENT*Point,Digits)
                                 ,2
