...
int start()
  {
   ...
   Lots = InitialLots * LotsFactor/LEVELS/2; 
...
//====================================Begin Placing Orders================================//
  ...
  // if(Hour() == 19 && Minute() == 0 && Seconds() == 0)CloseAll();
   ...
   if(/*total < 1 &&*/ lastTradeTime != Time[THIS_BAR] /*&& Hour() == 6 && Minute() == 0 && Seconds() == 0*/)
   {
    ...
    if(EnableStopLoss){
     ...
     for(cpt=1;cpt<=LEVELS;cpt++)
     {
         ...
         ticket=OrderSend(Symbol(),OP_BUYSTOP,Lots,NormalizeDouble(Ask+cpt*INCREMENT*Point,Digits),2,0,0,DoubleToStr(Ask,MarketInfo(Symbol(),MODE_DIGITS)),MagicNumber+2,0);
         // ticket=OrderSend(Symbol(),OP_BUYSTOP,Lots,NormalizeDouble(Ask+cpt*INCREMENT*Point,Digits),2,0,NormalizeDouble(Ask+1*INCREMENT*Point+TakeProfit*Point,Digits),DoubleToStr(Ask,MarketInfo(Symbol(),MODE_DIGITS)),MagicNumber+2,0); //  if(cpt==1)ticket=OrderSend(Symbol(),OP_BUYSTOP,Lots,NormalizeDouble(Ask+cpt*1*Point,Digits),2,0,0,DoubleToStr(Ask,MarketInfo(Symbol(),MODE_DIGITS)),MagicNumber+2,0);
       //  if(cpt>1)ticket=OrderSend(Symbol(),OP_BUYSTOP,Lots,NormalizeDouble(Ask+(cpt-1)*INCREMENT*Point,Digits),2,0,0,DoubleToStr(Ask,MarketInfo(Symbol(),MODE_DIGITS)),MagicNumber+2,0);
         ...
         ticket=OrderSend(Symbol(),OP_SELLSTOP,Lots,NormalizeDouble(Bid-cpt*INCREMENT*Point,Digits),2,0,0,DoubleToStr(Bid,MarketInfo(Symbol(),MODE_DIGITS)),MagicNumber+3,0);
   ...
   //  if(total<LEVELS)CloseAll();
    ...
    // BreakEven();
     //TrailingTakeProfit();
     if(AccountEquity()>AccountBalance()+Profit /*|| OrdersTotal() < LEVELS*2*/)CloseAll();
     //CheckBuySell();
